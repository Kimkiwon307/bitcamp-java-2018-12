    ...
    try (ServerSocket serverSocket = new ServerSocket(8888)) {
      ...
      while (true) {
        try (Socket socket = serverSocket.accept();
          ...
          //먼저 클라이언트가 보낸 세션 아이디를 읽는다. 웹서버가 웹브라우저에게 보내주는 세션값 = 쿠키
          ...
          if(sessionId == 0) {
            ...
            // 클라이언트의 세션 아이디로 기존에 저장된 결과 값을 가져온다.
            result = resultMap.get(sessionId); //오토 언박싱
          }
          ...
          // 계샨 결과를 세션 ID를 사용해서 서버에 저장한다.
          resultMap.put(sessionId, result);
          
          // 세션 아이디를 새로 발급했다면 클라이언트에게 알려준다
          if(isNewSessionId) {
