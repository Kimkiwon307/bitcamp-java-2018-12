...
public static void main(String[] args) {
  ...
  // 다른 패키지의 클래스를 사용하려면 패키지명까지 지정해야 한다.
  ...
  // => 서브 패키지는 해당이 안된다.
  // =>단 소스 코드를 읽을 때 어떤 클래스가 어떤 패키지에 있는지 바로 확인할 수 없는 불편함이 있따.
  //단 import명령은 package명령 다음에 와야 한다.
  // 클래스 선언 전에 와야 한다
  
  Score12 s2 = new Score12();
 // java.lang 패키지는 우리가 import하지 않아도 자동으로 import되어있다.
  // java.lang 패키지의 클래스를 사용할 때는 import 없이 사용하면 된다.
}
  }