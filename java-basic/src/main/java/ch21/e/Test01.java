...
public class Test01 {
  ...
  // => 단지 클래스 이름으로 예외를 직관적으로 알리기 위해 사용한ㄷ다.
  //
  static  class MyException extends Exception{

     // 보통 수퍼 클래스를 상속 받은 후 수퍼클래스와 같은 생성자를 만든다.
  ...
  public static void main(String[] args) throws Exception{
    ...
    //m1()에서 발생한 예욍 정보를 보면 예외 클래스 이름이 출력된다.
    // 예외 클래스 이름으로 어딩세ㅓ 왜 예외가 발생했는지 짐작할 수 있따.
    // 이런 이유로 사용자 정의 예외를 만들어 사용하는 것이다.
    
    // 대부분의 라이브러리나 프레임워크는 자신들만의 예외 클래스를 만들어 던지고 있따.    
    m1();
  }
