...
public interface B {
  ...
  //다음 메서드는 이 인터페이스를 구현하는 모든 클래스가 만드시 정의해야 한다.
  void m1();

  // 여러 번의 프로젝트가 끝난 후 
  // 새로 시작하는 프로젝트에 새 메서드가 필요함을 알게 되었다.
  // => 기존 규칙에 새 메서드를 넣으면 그 규칙에 따라 작성했떤 My1과 My2 클래스에도
  // 새 메서드를 추가해야 한다.
  // 그런데 기존 시스템을 사용하는 고객은 그 기능이 필요없다.
  // 시스템을 업그레이드 한다면 모를까!
  //=> 이런 상황일 때 예전에는 B 인터페이스를 상속 받아 새 메서드를 추가한 서브 인터페이스를 만들었따.
  ...
  // 디폴트 메서드를 만들라!
  //  => 구현된 메서드이기 때문에 B를 구현한 이전 My1과 My2는 영향을 받지 안흔다.
  // 이것이 디폴트 메서드의 목적이다.
  // => 물론 구현 메서드에 간단한 코드를 작성할 수 있찌만,
