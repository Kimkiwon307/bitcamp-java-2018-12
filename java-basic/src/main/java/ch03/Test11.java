...
public class Test11{
    ...
    // int arr2[5]; // 자바는 이런 무법을 지원하지 안흔다. 컴파일 오
     ...
     // 컴파일 오류가 발생한다.
    // arr[0] = 100; // 초기화 하지않고 사용했기 때ㅜㅁㄴ에 컴파일 오류 발생
     
     arr1 = null; //null? 0으로 설정한다. 즉 특정 메모리를 가리키지 않는 상태를 표시함.(주소없음)
     arr1[0] = 100; //arr1을 사용하기 전에 초기화 시켰기 때문에 컴파일 오류는 발생하지 안흔다.
     ...
     // 2) 배열을 생성하고 그 주소를 레퍼런스에 담는다.
      ...
      arr1[3] = 70;
      arr1[4] = 60;
     // arr1[-1] = 50; ArrayIndexOutOfBoundsException 예외 발생
