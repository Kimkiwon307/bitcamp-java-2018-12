...
public class Test01 {
	public static void main(String[] args) {
		...
		// =>  인스턴스의 생성과 관리를 담당한다.
		// => 각 개체가 의존하는 객체(dependency)를 주입한다.
		...
		//  => mvnrepository.com 에서 spring-context로 라이브러리를 검색한다.
		...
		// => 모든 스프링 IoC컨테이너는 이 규칙에 따라 IoC컨테이너가 정의 되어 있다.
		//
		// ApplicationContext 구현체(Implements, 인터페이스를 구현한 클래스 또는 그 클래스의 인스턴스)
		...
		//  ==> 자바 CLASSPATH?
		//		예를 들면 /User/eomjinyoug/git/bitcamp-2018-12/java-basic/bin/main
		...
		//		파일 경로이기 때문에 패키지와 패키지 사이에는 . 대신에 / 를 사용해야한다
		...
		//	==>	설정 파일 경로를 지정할 때 직접 파일 경로를 지정한다.
		// => 주의!
		//	설정 파일 경로르 지정할 때 URL 형식으로 지정해야 한다.
