...
// 스프링 IoC컨테이너가 객체를 생성한 후 보고를 하면
// 이 클래스는 생성된 객체에서 @AutWired가 붙은 메서드(세터)를 찾느다.
// 있따면 그 메서드를 호출하여 의존객체를 주입한다.
// 의존 객체가 없다면 그 의존 객체가 생성될때까지 별도로 담아둔다.
// 만약 의존 객체가 생성되는 순간 즉시 별도로 담아 둔 그 객체에 대해 셋터를 호출할 것이다
// 
// 
...
public class MyAutoWiredAnnotationBeanPostProcessor implements BeanPostProcessor {
	...
	// 파라미터 값이 준비되지 않아서 호출이 연기된 @Autowired 메서드를 기록한다
	...
	public Object postProcessAfterInitialization(Object bean, String beanName) throws BeansException {
		...
		for(Method m : methods) {
			//메서드에서 @Autowired 애노테이션 있는지 확인한다.
			Autowired anno	=m.getAnnotation(Autowired.class);
			if(anno == null) 
				continue;

			// 애노태이션이 있다면 이 셋터 메서드가 어떤 타입의 값을 원하는지 알아낸다.
