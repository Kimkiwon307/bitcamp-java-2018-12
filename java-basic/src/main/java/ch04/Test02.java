...
public class Test02{
    ...
    float r2 =(float)5 / (float)2 ;
     ...
     //    즉 개발자가 지시하지 않아도 내부적으로 같은 타입으로 만드는 것을 암시적 형변환 이라고한다.
       ...
       byte b1 = 20;
        byte b2 = 30;
        //byte b3 = b1+b2; // 암시적 형변환에 의해 b1, b2의 값은 int타입의 임시 메로리에 저장. 그런후 계산된다. 결과는 당연히 int이다.
      int x1 = b1 + b2;
        
        short s1 = 20;
        short s2 = 30;
       // short s3 = s1 +  s2 ; // 위와 같다. 계산하기전에 int임시메모리에 값이 저장된다
        int x2 = s1 + s2;
       ...
       //int i2 = i1 + f1; //컴파일오류
         ...
         double d1 = f3 + f4; // float과 float의 계산 결과는 float이다.
          ...
          // => 그런데 출력 결과를 보면 예상 결과와 다르게 나온다.
           ...
           //  부동소수점을 다룰 때 생기는 오차이다. 개발자가 제이할 수 없다. 
            System.out.println(d2);
            
            // 해결책?
            // => 부동소수점의 경우 계산 결과가 float의 자리수를 넘어ㅏ=갈 같으면
             //     아예 처음부터 double에 저장해 놓고 해라
