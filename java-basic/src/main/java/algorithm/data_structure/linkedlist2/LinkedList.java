...
public class LinkedList {
  ...
  //ArrayList 처럼 한 번에 큰 메모리를 준비할 필요가 읎다.
  // => 그러나 값 이외에 다음 노드와 이전도의 주소를 담기 위해 추가로 메모리를 사용한다.
  public void add(Object value) {
    ...
    // 새 노드의 이전으로 마지막 노드를 가리키게한다.
    node.prev = tail;

    // tail이 새로 추가된 노드를 가리키게한다.
    tail = node;
  ...
  //LinkedList에서만 쓰이는 Node 클래스를 중첩 클래스를 만든다.
