...
<jsp:setProperty name="b3" property="contents" value="내용이빈다"/>
<jsp:setProperty name="b3" property="viewCount" value="88"/>
<%--자바 원시 탑과 문자열을 제외한 속성의 값을 지정할 수 없다.
    따로 처리해야 한다. 
<jsp:setProperty name="b3" property="createdDate" value="2019-02-03"/>
--%>
...
==> 따로 taglib를 사용하여 라이브러리를 선어할 필요가 없다.
==> JSP에서 기본으로 제공하기 때문에 그대로 사용하면 된다.
==> 네임스페이스 이름은 jsp 이다.
      <jsp:태그명 ..../>
   jsp:useBean
=> JSP에서 사용할 객체를 생성할 때 사용할 수 있다.
=> 또는 보관소(ServletContext, HttpSession, ServeletRequest, PageContext)에 
   저장된 객체를 꺼낼 때도 사용한다.
=> 사용법 
...
=> 주요 속성
    scope 
          ...
          - 다음 4개의 값 중 한개를 지정할 수 있따. 값을 지정하지 안흥면 기본이 "page"이다
