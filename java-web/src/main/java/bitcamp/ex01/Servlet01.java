...
//
// 서블릿 실행 방법
// => http://서버주소:포트번호/웹에플리케이션이름/서브릿URL
//          http://localhost:8080/java-web/ex01/s01
// 서블릿 구동 과정
...
// 결론!
// => 특별한 옵션을 주지 안흥면 클라이언트가 최초로 요청햇을 때 서블릿 인스턴스를 생성한다.
// => 그리고 그 서블릿 인스턴스는 오직 한 개만 생성된다.
...
// => 클라이언트마다 구분되어야 할 데이터는 서블릿 인스턴스 변수에 보관해서는 아노딘다.
//     왜? 인스턴스는 모든 클라이언트가 굥유하기 때문이다.
...
public class Servlet01 implements Servlet{
  ...
  public void init(ServletConfig config) throws ServletException {
    ...
    // => 파라미터로 받은 ServletConfig 객체는 인스턴스 변수에 보관해 두었다가 필요할 때 사용한
    this.config = config;
    System.out.println("Servelet01.init(ServletConfig)");
  }
  ...
  @Override
  public void destroy() {
    // 웹 애플리케이션을 종료할 때 (서버종료포함) 이 서블릿이 만든 자원을 해제할 수 있도록
    // 서블릿 컨테이너가 이 메소드를 호출한다.
    // => init()에서 준비한 자우너을 보통 이메서드에서 해제한다.
